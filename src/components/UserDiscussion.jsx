...
const UserDiscussion = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const { data: notifications = [] } = await axiosInstance.get('/discussions/notifications');
          ...
          {isMobileMenuOpen && (
            <div className="lg:hidden mobile-menu-container bg-white rounded-xl shadow-xl p-3 sm:p-4 mb-4 sm:mb-6">
              <div className="flex flex-col space-y-3 sm:space-y-4">
                ...
                <button
                  onClick={() => {
                    setShowPopularHashtags(!showPopularHashtags);
                    setShowMyDoubts(false);
                    setShowMostLiked(false);
                    setShowBookmarks(false);
                  }}
                    ...
                    {popularHashtags.length > 0 ? (
                      popularHashtags.map((tag, index) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleLikeHashtag(tag.name)}
                ...
                <button
                  onClick={() => {
                    setShowMostLiked(!showMostLiked);
                    setShowMyDoubts(false);
                    setShowPopularHashtags(false);
                    setShowBookmarks(false);
                  }}
